		...
		<div class="schedule-container">
			...
			<div class="channel-track">
				...
				<css-grid :columns="currentGrid.columns" :rows="currentGrid.rows" :areas="currentGrid.areas">
					<css-grid-item :area="channel" class="channel-item" v-for="channel in channels">{{ channel }}</css-grid-item>
				</css-grid>
			</div>
...
export default {
	...
	data() {
		return {
			...
			timeSpan: 24,
			timeScale: 4,
			channels: ["Time", "Main", "Earth", "Knowhere", "Titan"],
			currentGrid: {
				columns: ["1fr"],
				rows: ["30px", "1fr", "1fr", "1fr", "1fr"],
				areas: [["Time"], ["Main"], ["Earth"], ["Knowhere"], ["Titan"]]
			}
		};
	},
	methods: {
		...
		programmeStartCoordinate(programme) {
			let minutes = new Date(programme.date * 1000).getMinutes();
			let hours = new Date(programme.date * 1000).getHours() * 60;
			let result = hours + minutes;
			let duration = programme.duration;
			return {
				left: result * this.timeScale + "px",
				width: duration * this.timeScale + "px"
			};
